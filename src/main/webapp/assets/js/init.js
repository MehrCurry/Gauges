define(
		...
		function(steelseries) {
			...
			function setValue(measurement) {
				...
				if (instrument != undefined && value != undefined
						&& !isNaN(value)) {
			...
			function convertToSection(ranges) {
				...
				for ( var i = 0; i < ranges.length; i++) {
					var a = ranges[i];
					var section = new steelseries.Section(
							parseInt(a.range.start.$), parseInt(a.range.end.$),
							"rgba({0},{1},{2},{3})".format(a.rgba.red,
									a.rgba.green, a.rgba.blue,
									a.rgba.alpha / 256.0))
					areas.push(section);
				}
				return areas;
